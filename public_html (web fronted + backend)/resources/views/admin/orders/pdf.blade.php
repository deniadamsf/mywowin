...
<div class="container">
    ...
    <div class="kop-logo">
        @if($branchSetting && $branchSetting->logo)
            ...
            <img src="{{ public_path('storage/' . $branchSetting->logo) }}" alt="Logo">
        @else
            <img src="{{ public_path('images/wwn-cr.png') }}" alt="Logo Default">
        @endif
    </div>
    <div class="kop-teks">
        {{-- Menampilkan Nama PT Dinamis dari Database --}}
        <h1>{{ $branchSetting->nama_pt ?? 'PT. WOWIN PURNOMO PUTERA' }}</h1>
        <p>{{ $branchSetting->alamat ?? 'Jl. Raya No.Km 07, Duwet, Ngetal, Kec. Pogalan, Trenggalek' }}</p>
        <p>Jawa Timur | Telp: {{ $branchSetting->no_telp ?? '0812-1630-1220' }} </p>
        {{-- Tampilkan website hanya jika ada di branchSetting --}}
        @if(isset($branchSetting->website))
            <p>website: {{ $branchSetting->website }}</p>
        @endif
    </div>
</div>
    ...
    <div class="info-section">
        <div class="info-group">
            ...
            <div class="info-item">
                ...
                <div class="info-value">{{ auth()->user()->nama_lengkap ?? 'Admin' }}</div>
            </div>
        </div>
                        ...
                        @foreach($order->orderItems as $item)
                            <div class="product-item">
                                <div class="product-name">
                                    @if($item->bundling_id && $item->bundling)
                                        {{ $item->bundling->nama_bundling }} (Bundling)
                                    @elseif($item->product)
    ...
    <div class="signature-section">
        <div class="signature-box">
            ...
            <p>{{ auth()->user()->nama_lengkap ?? 'Admin' }}</p>
            <p>Admin</p>
        </div>
    </div>
