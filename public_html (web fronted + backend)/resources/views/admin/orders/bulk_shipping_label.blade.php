...
<head>
    ...
    <title>Cetak Masal Label Thermal J&T ({{ $orders->count() }} Pesanan)</title>
...
<body>
    ...
    <div class="no-print-bar">
        <div>
            <div style="font-weight: bold; font-size: 13px;">Cetak Masal Label Thermal J&T (Total: {{ $orders->count() }} Label)</div>
            <div style="font-size: 10px; color: #94a3b8;">Format Kertas: 100x150 mm (Roll Sticker Thermal)</div>
        </div>
    ...
    <!-- LOOP SEMUA LABEL PESANAN -->
    @foreach($orders as $order)
        @php
            $adminCabang = $order->user->kantor_cabang ?? 'Trenggalek';
            $branchSetting = $branchSettings[$adminCabang] ?? null;
        @endphp
        <div class="label-container {{ !$loop->last ? 'page-break' : '' }}">
            ...
            <div class="barcode-area">
                <div class="barcode-svg-wrapper">
                    <svg class="barcode-svg" data-resi="{{ $order->no_resi ?: $order->invoice_number }}"></svg>
                </div>
