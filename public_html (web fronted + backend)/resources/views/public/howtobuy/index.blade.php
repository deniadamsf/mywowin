...
<br>
<h1 class="text-2xl font-semibold text-black text-center">Cara Belanja</h1>
<!-- Accordion 5 Pertanyaan Pokok Cara Belanja di MyWowin -->
<div class="max-w-3xl mx-auto mt-6">
  <div class="bg-white shadow-lg rounded-xl divide-y divide-gray-200">
      @php
          $questions = [
              [
                  'q' => 'Bagaimana cara memulai belanja di MyWowin?',
                  'a' => 'Kamu bisa langsung membuka situs resmi kami di https://mywowin.com. Setelah itu, kamu bisa mulai menjelajahi berbagai produk kecap berkualitas yang tersedia. Gunakan fitur pencarian untuk mencari produk tertentu, atau pilih dari kategori yang sudah disediakan. Setelah menemukan produk yang kamu inginkan, klik produk tersebut untuk melihat detailnya.'
              ],
              [
                  'q' => 'Apakah saya perlu membuat akun untuk berbelanja?',
                  'a' => 'Tidak wajib, kamu tetap bisa berbelanja sebagai tamu. Namun, kami sangat menyarankan untuk membuat akun agar kamu dapat menyimpan riwayat pesanan, melacak status pengiriman, dan mendapatkan penawaran atau diskon khusus. Proses pendaftaran sangat mudah dan cepat, cukup menggunakan email aktif dan nomor telepon.'
              ],
              [
                  'q' => 'Bagaimana cara memilih dan menambahkan produk ke keranjang?',
                  'a' => 'Setelah membuka halaman produk yang kamu minati, kamu akan melihat informasi lengkap seperti harga, ukuran, dan deskripsi produk. Pilih jumlah produk yang ingin dibeli, lalu klik tombol “Tambah ke Keranjang”. Produk akan otomatis masuk ke keranjang belanja yang bisa kamu akses di pojok kanan atas halaman.'
              ],
              [
                  'q' => 'Bagaimana proses pembayaran dilakukan?',
                  'a' => 'Setelah semua produk masuk ke keranjang, klik ikon keranjang untuk melihat daftar belanjaanmu. Lanjutkan ke proses checkout dengan mengisi data pengiriman secara lengkap. Setelah itu, pilih metode pembayaran yang tersedia seperti transfer bank atau e-wallet. Ikuti instruksi pembayaran yang muncul hingga kamu mendapatkan notifikasi bahwa pembayaran berhasil.'
              ],
              [
                  'q' => 'Berapa lama pengiriman setelah pembayaran?',
                  'a' => 'Setelah pembayaran kamu dikonfirmasi, tim kami akan segera memproses pesananmu dalam waktu 1x24 jam kerja. Waktu pengiriman tergantung lokasi tujuan. Untuk wilayah Jawa biasanya membutuhkan 2–4 hari kerja, sedangkan luar pulau bisa memakan waktu 5–7 hari kerja. Kamu juga akan mendapatkan nomor resi untuk melacak pengiriman secara online.'
              ],
          ];
      @endphp
      ...
      @foreach ($questions as $index => $item)
          <div x-data="{ open: {{ $index === 0 ? 'true' : 'false' }} }" class="px-6 py-4">
              <button @click="open = !open" class="w-full text-left flex justify-between items-center focus:outline-none">
                  <span :class="open ? 'text-green-600 font-semibold' : 'text-gray-800'" class="transition duration-200">
                      {{ $item['q'] }}
                  </span>
                  <svg :class="open ? 'transform rotate-180 text-green-600' : 'text-gray-600'" class="w-4 h-4 transition duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
